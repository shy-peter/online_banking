      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {paymentMethodTypes.map((type) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className={`p-3 rounded-lg ${
                  selectedType?.$id === type.$id ? 'bg-primary-100' : 'bg-gray-100'
                }`}>
                  {getIconComponent(type.icon || 'CreditCard')}
                </div>
                <div>
                  <h3 className="font-semibold text-gray-900">{type.name}</h3>
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getCategoryColor(type.category)}`}>
            ...
            {type.requiredFields && (
              <div className="mt-3">
                <p className="text-xs text-gray-500 mb-1">Required Information:</p>
                <div className="flex flex-wrap gap-1">
                  {JSON.parse(type.requiredFields).map((field: string, index: number) => (
